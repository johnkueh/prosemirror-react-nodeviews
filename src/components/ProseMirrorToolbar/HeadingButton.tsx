...
import { markActive, ToolbarButton } from "../ProseMirrorToolbar";
...
const HeadingButton: React.FC<Props> = () => {
  ...
  return (
    <Flex position="relative">
      <ToolbarButton
        icon={faHeading}
        active={Boolean(markActive(editorView?.state, schema?.marks.underline))}
        onClick={onOpen}
      />
      {isOpen && (
        <>
          ...
          >
            ...
            <MenuItem
              onClick={() => {
                setBlockTypeCallback(schema?.nodes.heading, {
                  level: 1
                });
                onClose();
              }}
